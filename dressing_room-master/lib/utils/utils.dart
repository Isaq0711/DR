...
String nuvem = '191.101.78.201';

String? selectedServerAddress = nuvem;

String port = '5000';

Future<Uint8List> removeBg(String imagePath, bool save, dynamic format) async {
  var request = http.MultipartRequest(
      "POST", Uri.parse('http://$selectedServerAddress:$port/removebg'));
...
Future<Uint8List?> pickImageOriginal(ImageSource source) async {
  final ImagePicker _imagePicker = ImagePicker();
  XFile? _file = await _imagePicker.pickImage(source: source);
...
Future<Uint8List?> pickImageinicial916(
    {Uint8List? imageData, ImageSource? source}) async {
  XFile? _file;
  Uint8List? imageBytes;
